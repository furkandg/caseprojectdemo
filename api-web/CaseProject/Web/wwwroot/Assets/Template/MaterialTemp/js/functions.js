...
$(document).on("click", ".checkbox", function () {
    ...
    let cbox = $(this).find("[type='checkbox']");
    if ($(cbox).is(":checked")) {
        $(this).find("[type='hidden']").val("true");
    }
    else {
        $(this).find("[type='hidden']").val("false");
    }
});
...
$(document).on("click", ".pop-over", function () {
    let isPopOver=$(this).data("ispopover");
    if (isPopOver != 1) {
        let content = $(this).find(".popover-content").html();
        let _title = $(this).data("original-title");
        let _placement = $(this).data("placement");
        $(this).popover({
            container: 'body',
            html: true,
            content: content,
...
$(document).ready(function () {
    ...
    (function () {
        ...
        let layoutStatus = localStorage.getItem('ma-layout-status');
    ...
    $('body').on('click', '[data-clear="notification"]', function (e) {
        e.preventDefault();
        let self = this;
        $.get("/Bildirim/SetAktifAll/", function (result) {
            if (result.Status == "OK") {
                $(".tmn-counts").html(0);
                let x = $(this).closest('.listview');
    ...
    if ($('.dropdown')[0]) {
        ...
        $('body').on('click', '.dropdown.open .dropdown-menu', function (e) {
            ...
            if ($(this).find(".load-modal").length == 0)
                e.stopPropagation();
        });
    ...
    if ($('#todo-lists')[0]) {
        ...
        $('body').on('click', '.add-tl-actions > a', function (e) {
            e.preventDefault();
            let x = $(this).closest('#add-tl-item');
            let y = $(this).data('tl-action');
    ...
    */
    ...
    if ($('.input-slider')[0]) {
        $('.input-slider').each(function () {
            let isStart = $(this).data('is-start');
    ...
    if ($('.color-picker')[0]) {
        $('.color-picker').each(function () {
            let colorOutput = $(this).closest('.cp-container').find('.cp-value');
    ...
    //Actions
    if ($('.on-select')[0]) {
        let checkboxes = '.lv-avatar-content input:checkbox';
        let actions = $('.on-select').closest('.lv-actions');
    ...
    if ($('.login-content')[0]) {
        ...
        $('body').on('click', '.login-navigation > li', function () {
            let z = $(this).data('block');
            let t = $(this).closest('.lc-block');
    ...
    if ($('[data-action="clear-localstorage"]')[0]) {
        let cls = $('[data-action="clear-localstorage"]');
    ...
    if ($('.typeahead')[0]) {
        ...
        let statesArray = ['Alabama', 'Alaska', 'Arizona', 'Arkansas', 'California',
    ...
    $('body').on('click', '[data-skin]', function () {
        let currentSkin = $('[data-current-skin]').data('current-skin');
