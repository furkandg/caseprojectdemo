...
jQuery.fn.extend({
    FormFactory: function (params) {
        let ajaxPost = function (ajax, callback, errorback) {
            let _data = JSON.parse(JSON.stringify(ajax.data));
            loadingLoader.start();
            $.ajax({
                type: ajax.type || "POST",
                //crossDomain: true,
                ...
                success: function (msg) {
                    let response = msg.hasOwnProperty("d") ? msg.d : msg;
        ...
        let ajax = params.ajax || undefined;
        if (params.url) {
            ajax = { url: params.url };
        }
        ...
        $(this).submit(function (evt) {
            evt.preventDefault();
            let postData = $(this).serializeForm();
            if (!$(this).valid()) {
                console.info($(this).data("validator"), postData);
                return false;
            }
            if (params.customValidation) {
                let customValid = params.customValidation(postData);
                if (!customValid) {
                    return false;
                }
            }
            if (params.addExtraData) {
                let valid = params.addExtraData(postData);
                if (valid != undefined) {
                    if (!valid) {
...
jQuery.fn.extend({
    AutoCompleter: function (params) {
        var _self = this;
        $(this).attr("data-live-search", "true");
        $(this).attr("data-live-search-liveSearchNormalize", "true");
        $(this).attr("data-live-search-normalize", "true");
        $(this).attr("data-live-search-placeholder", params.placeholder || "Arama metnini giriniz.");
        $(this).selectpicker();
    ...
    AdresFormGroup: function (params) {
        var _container = $(this);
        var ilceIdName = params.ilceIdName || "#ilcekn";
        var mahalleIdName = params.mahalleIdName || "#mahallekn";
        var yolIdName = params.yolIdName || "#yolkn";
        var loadSelectOptions = function (id, data) {
            var selectItem = $(id);
            for (var i = 0; i < data.length; i++) {
                var item = data[i];
                selectItem.append('<option value="' + item.Value + '">' + item.Text + '</option>')
            }
            if (params.selectpicker)
                selectItem.selectpicker('refresh');
        }

        var ilceBox = _container.find(ilceIdName);
        if (params.selectpicker) ilceBox.AutoCompleter({ placeholder: "İlçe " });
        var mahalleBox = _container.find(mahalleIdName);
        if (params.selectpicker) mahalleBox.AutoCompleter({ placeholder: "Mahalle" });
        var yolBox = _container.find(yolIdName);
        if (params.selectpicker) yolBox.AutoCompleter({ placeholder: "Cadde Sokak" });
        ilceBox.change(function () {
            var ilceID = $(this).val();

            mahalleBox.html("<option>Mahalle Seçiniz</option>");
            yolBox.html("<option>Cadde Sokak Seçiniz</option>");
            if (params.selectpicker) yolBox.selectpicker('refresh');
            $.get((params.serviceUrl || "/Adres") + "/GetAdresMahalle/" + ilceID, function (data) {
                loadSelectOptions(mahalleBox, data)
            })
        });
        mahalleBox.change(function () {
            yolBox.html("<option>Cadde Sokak Seçiniz</option>");
            var mahalleID = $(this).val();
            if (params.selectpicker) yolBox.selectpicker('refresh');
            $.get((params.serviceUrl || "/Adres") + "/GetAdresYol/" + mahalleID, function (data) {
                loadSelectOptions(yolBox, data)
            })
        });
...
let ilceManager = function () {
    ...
    if ($_configData.loginedUserRole == "KurumAdministrator" || $_configData.loginedUserRole == "KurumUser") {
        for (let i = 0; i < ilcelerAll.length; i++) {
            let ilceItem = ilcelerAll[i];
    ...
    let _CheckBoxListFor = function (params) {

        let firstname = params.firstname;
        let lastname = params.lastname;
        let selectedSource = params.selectedSource || null;
        let elem = params.elem;

        let sayhidden = 0;
        let hiddens = $('<div class="hiddens"></div>');
        let _container = $('<div class="group-container"></div>');

        let rnd = Math.floor((Math.random() * 1000000000) + 1);
        let avrupaCount = 0;
        let avrupaPanel = $('<div class="panel panel-collapse"></div>').appendTo(_container);
        ...
        let avrupaTabPanel = $('<div id="collapse1' + rnd + '" class="collapse" role="tabpanel" aria-labelledby="heading1"></div>').appendTo(avrupaPanel);
        let avrupaPanelBody = $(' <div class="panel-body"><input type="checkbox" class="select-all"><label>Tümünü Seç</label></div>').appendTo(avrupaTabPanel);

        let anadoluCount = 0;
        let anadoluPanel = $('<div class="panel panel-collapse"></div>').appendTo(_container);
        $('<div class="panel-heading" role="tab" id="heading1">' +
                        '<h4 class="panel-title">' +
        ...
        let anadoluTabPanel = $('<div id="collapse2' + rnd + '" class="collapse" role="tabpanel" aria-labelledby="heading1"></div>').appendTo(anadoluPanel);
        let anadoluPanelBody = $(' <div class="panel-body"><input type="checkbox" class="select-all"><label>Tümünü Seç</label></div>').appendTo(anadoluTabPanel);

        for (index in ilceler) {
            let item = ilceler[index];
            let selectedText = "";
            ...
            if (selectedSource != null) {
                ...
                for (let i = 0; i < selectedSource.length; i++) {
                    let sourceItem = selectedSource[i].Value;
            ...
            let cboxItem = $('<label class="mvc-checkboxlistfor">' +
        ...
        _container.find(".mvc-listcheckbox").click(function () {
            ...
            let sahip = label.parent();

            let chkbox = $(this);
            let val = chkbox.val();
            let firstname = chkbox.attr("data-firstname");
            let lastname = chkbox.attr("data-lastname");
    ...
    let _DropdownListFor = function (params) {
        let _select = $('<select id="' + (params.id || "") + '" name="' + (params.name || "") + '"></select>');
        for (index in ilceler) {
            let item = ilceler[index];
...
let UniqueValueManager = function () {
    let MakeUniqueValues = function (uniquevalues) {
        var ret = [];
        for (var i = 0; i < uniquevalues.length; i++) {
            var uniqueValueItem = uniquevalues[i];
            for (var j = 0; j < uniqueValueItem.value.length; j++) {
                var v1Item = uniqueValueItem.value[j];
                for (var k = 0; k < uniqueValueItem.value2.length; k++) {
                    var v2Item = uniqueValueItem.value2[k];
                    if (uniqueValueItem.value3) {
                        for (var l = 0; l < uniqueValueItem.value3.length; l++) {
                            var v3Item = uniqueValueItem.value2[k];
                            var newUniqueValue = $.extend(null, uniqueValueItem);
                            newUniqueValue["value"] = v1Item;
                            newUniqueValue["value2"] = v2Item;
                            newUniqueValue["value3"] = v3Item;
                            ret.push(newUniqueValue);
                        }
                    } else {
                        var newUniqueValue = $.extend(null, uniqueValueItem);
                        newUniqueValue["value"] = v1Item;
    ...
    return { MakeUniqueValues: MakeUniqueValues }
}();
...
let PageManager = function () {
    ...
    let hiddens = $("<div></div>");
    let parentContainer = "#map";
    let pageContainer = 'form-container';
    ...
    let loadBeforePage = function (page) {
    ...
    let open = function (params) {
        ...
        if (Array.isArray(params.headerActions)) {
            for (let i = 0; i < params.headerActions.length; i++) {
...
let Credential = function () {
    this.Email = params.Email || "";
    this.Password = params.Password || "";
}
let Identy = function (params) {
    this.Id = params.Id || "";
    this.Email = params.Email || "";
...
let ServiceResponse = function (params) {
...
let LoginManager = function () {
    ...
    let _getlogined = function () {
        return new Identy(CookieManager.Set("login"));
    }
    let _hasRole = function (roles, noadmin) {
        let role = _getRole();
    ...
    let getLoginedUserName = function () {
        return $_configData._LOGINED_USER.Name;
    }
    let getLoginedUserID = function () {
...
let RendererTemplateManager = function () {
    let rendererTemplates = [
       {
           key: "default",
           label: "Varsayılan"
       },
    ...
    let _set = function (temp) {
        localStorage.setItem("rendererTemplate", temp);
    }
    let _has = function (arr) {
        let _template = _get();
        if (!arr) { return true; }
...
if (isMobile()) {
    ...
    let setScroll = function () {
        ...
        let _offset = $(this).parents(".form-group").offset();
        let toScroll = _offset.top - $container.offset().top + $container.scrollTop();
        $container.animate({
            scrollTop: toScroll
        }, 100);
    }
    let removeScroll = function () {
...
$(document).on("keyup input", ".form-control", function (e) {
    if (e.which == 13) {
        let _next = $(this).parents(".form-group").nextAll('.form-group:first').find(".form-control");
        _next.focus();
    }
});
